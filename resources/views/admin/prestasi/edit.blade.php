                    ...
                    <div class="bg-white rounded-2xl border border-gray-100 p-6">
                        <h2 class="text-sm font-bold text-gray-700 mb-5 flex items-center gap-2">
                            <span class="w-5 h-5 rounded-md bg-primary/10 flex items-center justify-center">
                                <svg class="w-3 h-3 text-primary" fill="none" viewBox="0 0 24 24" stroke-width="2"
                                    stroke="currentColor">
                        ...
                        <div class="space-y-4">
                            {{-- Judul --}}
                            <div>
                                <label class="block text-xs font-semibold text-gray-600 mb-1.5">Judul Prestasi <span
                            ...
                            {{-- Nama Lomba --}}
                            <div>
                                <label class="block text-xs font-semibold text-gray-600 mb-1.5">Nama Lomba / Kompetisi <span
                                        class="text-red-400">*</span></label>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-xs font-semibold text-gray-600 mb-1.5">Tingkat <span
                                            class="text-red-400">*</span></label>
                                    <select name="tingkat"
                                        ...
                                        @foreach (['kecamatan' => 'Kecamatan', 'kabupaten' => 'Kabupaten / Kota', 'provinsi' => 'Provinsi', 'nasional' => 'Nasional', 'internasional' => 'Internasional'] as $val => $label)
                                            <option value="{{ $val }}"
                                                {{ old('tingkat', $prestasi->tingkat) == $val ? 'selected' : '' }}>
                                                {{ $label }}</option>
                                        @endforeach
                                    </select>
                                    @error('tingkat')
                                ...
                                <div>
                                    <label class="block text-xs font-semibold text-gray-600 mb-1.5">Juara / Penghargaan
                                    ...
                                    <select name="juara"
                                        ...
                                        @foreach (['Juara 1' => 'Juara 1', 'Juara 2' => 'Juara 2', 'Juara 3' => 'Juara 3', 'Harapan 1' => 'Juara Harapan 1', 'Harapan 2' => 'Juara Harapan 2', 'Harapan 3' => 'Juara Harapan 3', 'Finalis' => 'Finalis', 'Peserta Terbaik' => 'Peserta Terbaik'] as $val => $label)
                                            <option value="{{ $val }}"
                                                {{ old('juara', $prestasi->juara) == $val ? 'selected' : '' }}>
                                                {{ $label }}</option>
                                        @endforeach
                                    </select>
                                    @error('juara')
                            ...
                            <div>
                                ...
                                <textarea name="deskripsi" rows="4" placeholder="Ceritakan lebih lanjut tentang prestasi ini..."
                                    class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition placeholder:text-gray-300 resize-none">{{ old('deskripsi', $prestasi->deskripsi) }}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl border border-gray-100 p-6">
                        <h2 class="text-sm font-bold text-gray-700 mb-5 flex items-center gap-2">
                            <span class="w-5 h-5 rounded-md bg-blue-50 flex items-center justify-center">
                                <svg class="w-3 h-3 text-blue-500" fill="none" viewBox="0 0 24 24" stroke-width="2"
                                    stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        <div class="space-y-4">
                            <div>
                                <label class="block text-xs font-semibold text-gray-600 mb-1.5">Nama Peserta <span
                                        class="text-red-400">*</span></label>
                                <input type="text" name="nama_peserta"
                                    value="{{ old('nama_peserta', $prestasi->nama_peserta) }}"
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" name="nis_nip" value="{{ old('nis_nip', $prestasi->nis_nip) }}"
                    ...
                    <div class="bg-white rounded-2xl border border-gray-100 p-6">
                        <h2 class="text-sm font-bold text-gray-700 mb-1 flex items-center gap-2">
                            <span class="w-5 h-5 rounded-md bg-purple-50 flex items-center justify-center">
                                <svg class="w-3 h-3 text-purple-500" fill="none" viewBox="0 0 24 24" stroke-width="2"
                                    stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        <p class="text-xs text-gray-400 mb-5">Opsional — biarkan kosong untuk menggunakan judul & deskripsi
                            utama.</p>
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="text" name="meta_title"
                                    value="{{ old('meta_title', $prestasi->meta_title) }}"
                                    placeholder="Judul untuk mesin pencari"
                                    class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition placeholder:text-gray-300">
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-gray-600 mb-1.5">Meta Description</label>
                                <textarea name="meta_description" rows="2" placeholder="Deskripsi singkat untuk mesin pencari..."
                                    class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition placeholder:text-gray-300 resize-none">{{ old('meta_description', $prestasi->meta_description) }}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl border border-gray-100 p-6">
                        ...
                        <div class="bg-gray-50 rounded-xl px-4 py-3 mb-5 space-y-1.5">
                            <div class="flex items-center justify-between text-xs">
                                ...
                                <span class="font-mono text-gray-600 font-medium">#{{ $prestasi->id }}</span>
                            </div>
                            <div class="flex items-center justify-between text-xs">
                                <span class="text-gray-400">Dibuat</span>
                                <span class="text-gray-600">{{ $prestasi->created_at->format('d M Y') }}</span>
                            </div>
                            <div class="flex items-center justify-between text-xs">
                                <span class="text-gray-400">Dilihat</span>
                                <span class="text-gray-600">{{ number_format($prestasi->jumlah_dilihat) }}x</span>
                            </div>
                        </div>
                            ...
                            <label class="flex items-center gap-3 cursor-pointer group">
                                <div class="relative">
                                    <input type="checkbox" name="is_featured" value="1"
                                        {{ old('is_featured', $prestasi->is_featured) ? 'checked' : '' }}
                                        class="sr-only peer">
                                    <div
                                        class="w-10 h-6 rounded-full bg-gray-200 peer-checked:bg-primary transition-colors">
                                    </div>
                                    <div
                                        class="absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white shadow transition-transform peer-checked:translate-x-4">
                                    </div>
                                </div>
                                <div>
                    ...
                    <div class="bg-white rounded-2xl border border-gray-100 p-6">
                        ...
                        @if ($prestasi->foto)
                            <div class="mb-4">
                                ...
                                <img src="{{ asset('storage/' . $prestasi->foto) }}" alt="{{ $prestasi->judul }}"
                                    class="w-full h-40 object-cover rounded-xl border border-gray-100" id="current-foto">
                            </div>
                        @endif
                        ...
                        <div id="drop-zone"
                            ...
                            <div id="drop-placeholder">
                                <div
                                    class="w-12 h-12 rounded-xl bg-gray-100 group-hover:bg-green-100 flex items-center justify-center mx-auto mb-3 transition">
                                    <svg class="w-6 h-6 text-gray-300 group-hover:text-primary transition" fill="none"
                                ...
                                <p class="text-sm text-gray-400 group-hover:text-gray-600 transition">
                                    {{ $prestasi->foto ? 'Ganti foto' : 'Klik untuk pilih foto' }}
                                </p>
    ...
    <script>
        ...
        function showPreview(file) {
            ...
            reader.onload = e => {
                ...
                if (currentFoto) currentFoto.classList.add('opacity-40');
            };
