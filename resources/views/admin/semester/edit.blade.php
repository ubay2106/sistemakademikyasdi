...
<div class="flex items-center gap-3 mb-6">
    ...
    <div>
        <h1 class="text-lg font-bold text-gray-800 leading-none">Edit Semester</h1>
        <p class="text-xs text-gray-400 mt-0.5">Perbarui detail semester</p>
    </div>
</div>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <label class="cursor-pointer">
                                <input type="radio" name="nama" value="Ganjil" class="peer sr-only"
                                    {{ old('nama', $semester->nama) === 'Ganjil' ? 'checked' : '' }}>
                            ...
                            <label class="cursor-pointer">
                                <input type="radio" name="nama" value="Genap" class="peer sr-only"
                                    {{ old('nama', $semester->nama) === 'Genap' ? 'checked' : '' }}>
                                <div class="flex items-center gap-3 border-2 border-gray-200 rounded-xl px-4 py-3.5 transition
                ...
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0
                        {{ $semester->is_active
                            ...
                            : ($semester->nama === 'Ganjil' ? 'bg-blue-50 text-blue-400' : 'bg-purple-50 text-purple-400') }}">
                        @if($semester->nama === 'Ganjil')
                    ...
                    <div>
                        <p class="font-bold text-sm text-gray-800">Semester {{ $semester->nama }}</p>
                        @if($semester->tahunAjaran)
                            <p class="text-xs text-gray-400 mt-0.5">{{ $semester->tahunAjaran->nama }}</p>
                        @endif
                    ...
                    <div class="flex items-start gap-3 border-2 border-gray-200 rounded-xl p-4 transition
                        ...
                        <div class="relative mt-0.5">
                            <input type="checkbox" name="is_active" value="1"
                                class="peer sr-only" {{ old('is_active', $semester->is_active) ? 'checked' : '' }}>
            ...
            <div class="bg-amber-50 rounded-2xl border border-amber-100 p-5">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-amber-600 leading-relaxed">Hanya boleh ada <strong>satu</strong> semester aktif dalam satu waktu. Mengaktifkan semester ini akan menonaktifkan yang lain secara otomatis.</p>
                    </div>
                </div>
            </div>
