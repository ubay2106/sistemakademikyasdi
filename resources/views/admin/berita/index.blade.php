...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
    ...
    <div class="bg-white rounded-xl border border-gray-100 px-4 py-3 flex items-center gap-3">
        ...
        <div>
            <p class="text-lg font-bold text-gray-800 leading-none">{{ $beritas->where('status', 'published')->count() }}</p>
    ...
    <div class="bg-white rounded-xl border border-gray-100 px-4 py-3 flex items-center gap-3">
        ...
        <div>
            <p class="text-lg font-bold text-gray-800 leading-none">{{ $beritas->where('status', 'draft')->count() }}</p>
    ...
    <div class="bg-white rounded-xl border border-gray-100 px-4 py-3 flex items-center gap-3">
        ...
        <div>
            <p class="text-lg font-bold text-gray-800 leading-none">{{ $beritas->where('status', 'archived')->count() }}</p>
        ...
        <table class="w-full">
            <thead>
                <tr class="border-b border-gray-100 bg-gray-50/70">
                    ...
                    <th class="text-left text-xs font-semibold text-gray-400 uppercase tracking-wider px-4 py-4 hidden md:table-cell">Kategori</th>
                    <th class="text-left text-xs font-semibold text-gray-400 uppercase tracking-wider px-4 py-4 hidden lg:table-cell">Tanggal</th>
                    <th class="text-left text-xs font-semibold text-gray-400 uppercase tracking-wider px-4 py-4 hidden lg:table-cell">Status</th>
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50/50 transition group">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div class="min-w-0">
                                <div class="flex items-center gap-1.5">
                                    <p class="font-semibold text-gray-800 text-sm truncate max-w-[220px] group-hover:text-primary transition">{{ $item->judul }}</p>
                                    @if($item->is_featured)
                                ...
                                <p class="text-gray-400 text-xs mt-0.5 line-clamp-1 max-w-[220px]">{{ Str::limit(strip_tags($item->isi), 60) }}</p>
                            </div>
                        </div>
                    </td>
                    ...
                    <td class="px-4 py-4 hidden md:table-cell">
                        @if($item->kategori)
                            <span class="inline-block text-xs font-semibold bg-primary/10 text-primary px-3 py-1 rounded-full">
                                {{ $item->kategori->nama }}
                            </span>
                        @else
                            <span class="inline-block text-xs font-semibold bg-gray-100 text-gray-400 px-3 py-1 rounded-full">
                                Umum
                            </span>
                        @endif
                    </td>
                    ...
                    <td class="px-4 py-4 hidden lg:table-cell">
                        ...
                        @elseif($item->status === 'archived')
                            <span class="inline-flex items-center gap-1.5 text-xs font-semibold bg-amber-50 text-amber-600 px-3 py-1 rounded-full">
